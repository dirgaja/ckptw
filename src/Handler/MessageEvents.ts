...
export const emitPollCreation = async (
  m: any,
  ev: any,
  self: any,
  core: any
  ...
  m.pollValues = m.message.pollCreationMessage.options.map(
    (x: any) => x.optionName
  );
  m.pollSingleSelect = Boolean(
    m.message.pollCreationMessage.selectableOptionsCount
  );
  ev?.emit(
    Events.Poll,
    m,
    new Ctx({ used: { poll: m.content }, args: [], self: self, client: core })
  );
};
