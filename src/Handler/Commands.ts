...
export = async (self: ICtxSelf) => {
  ...
  const hasHears = Array.from(self.hearsMap.values()).filter(
    (x) =>
      x.name === m.content ||
      x.name === m.messageType ||
      new RegExp(x.name).test(m.content as string) ||
      (Array.isArray(x.name) ? x.name.includes(m.content) : false)
  );
  if (hasHears.length)
    return hasHears.map((x) =>
      x.code(
        new Ctx({
          used: { hears: m.content },
          args: [],
          self,
          client: self.core,
        })
  ...
  let allCommandsValue = Array.from(
    cmd?.values() as unknown as ArrayLike<unknown>
  );
  ...
  if (commandDetail.length)
    commandDetail.map((x) =>
      x.code(
        new Ctx({
          used: { prefix: selectedPrefix, command },
          args,
          self,
          client: self.core,
        })
