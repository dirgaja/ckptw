...
export class GroupData {
  ...
  async membersCanAddMemberMode(mode: "on" | "off"): Promise<void> {
    return await this.ctx._client.groupMemberAddMode(
      this.jid,
      mode === "on" ? "all_member_add" : "admin_add"
    );
  }
  ...
  > {
    return await this.ctx._client.groupParticipantsUpdate(
      this.jid,
      members,
      action
    );
  ...
  async pendingMembersUpdate(members: string[], action: "reject" | "approve") {
    return this.ctx._client.groupRequestParticipantsUpdate(
      this.jid,
      members,
      action
    );
