...
export class Ctx implements ICtx {
  ...
  constructor(options: ICtxOptions) {
    ...
    if (this._sender.jid)
      this._sender.decodedJid = decodeJid(this._sender.jid as string);
  ...
  async sendMessage(
    ...
    if (this._self.autoMention) {
      let matchMention = (content as any).text.match(
        /(@[^](?![a-zA-Z]).\d*[$]*)/gm
      );
      if (matchMention) {
  ...
  async getMediaMessage(
    msg: IMessageInfo,
    type: "buffer" | "stream"
  ): Promise<Buffer | import("stream").Transform | null> {
    try {
      let buffer = await downloadMediaMessage(
        ...
        {},
        {
          logger: this._self.logger as any,
  ...
  async block(jid?: string): Promise<void> {
    if (jid) {
      ...
      await this._client.updateBlockStatus(
        decodeJid(this.id as string),
        "block"
      );
    }
  }
  ...
  get quoted() {
    let quotedMessage = this._msg.message?.extendedTextMessage?.contextInfo
      ?.quotedMessage as WAProto.IMessage | undefined;
    return {
      ...quotedMessage,
      media: {
        toBuffer: async () => {
          try {
            let type = this.getContentType(
              quotedMessage
            ) as keyof typeof quotedMessage;
            let stream = await this.downloadContentFromMessage(
              quotedMessage?.[type] as any,
              (type as string).slice(0, -7) as any
            );
  ...
  sendInteractiveMessage(
    jid: string,
    content: IInteractiveMessageContent,
        ...
        for (let i = 0; i < matchMention.length; i++) {
          if (matchMention[i].match(/^@\d/gm)) {
            ...
            if (contentReal.contextInfo?.mentionedJid) {
              contentReal.contextInfo?.mentionedJid.push(
                `${num}@s.whatsapp.net`
              );
            } else {
      ...
      jid,
      {
        viewOnceMessage: {
          message: {
            ...
            interactiveMessage:
              proto.Message.InteractiveMessage.create(contentReal),
          },
        },
      },
      options as any
