...
export class Client {
  ...
  onMessage() {
    this.core?.ev.on("messages.upsert", async (m: any) => {
      ...
      this.ev?.emit(
        Events.MessagesUpsert,
        m,
        new Ctx({
          used: { upsert: m.content },
          args: [],
          self,
          client: this.core,
        })
   ...
   */
  hears(
    query: string | Array<string> | RegExp,
    callback: (ctx: Ctx) => Promise<any>
  ) {
  ...
  async fetchBio(
    jid?: string
  ): Promise<undefined | { setAt: Date; status: undefined | string }> {
  ...
  async launch() {
    ...
    if (this.usePairingCode && !this.core.authState.creds.registered) {
      if (this.printQRInTerminal)
        throw new Error(
          "[ckptw] if you are set the usePairingCode to true then you need to set printQRInTerminal to false."
        );
      if (!this.phoneNumber)
        throw new Error(
          "[ckptw] the phoneNumber options are required if you are using usePairingCode"
        );

      this.phoneNumber = this.phoneNumber.replace(/[^0-9]/g, "");
      if (!this.phoneNumber.length)
        throw new Error("[ckptw] invalid phoneNumber");

      if (
        !Object.keys(PHONENUMBER_MCC).some((v) =>
          this.phoneNumber!.startsWith(v)
        )
      )
        throw new Error(
          "[ckptw] phoneNumber format must be like: 62xxx (starts with the country code)"
        );
